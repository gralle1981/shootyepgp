...
function sepgp:buildMenu()
  if not (options) then
    ...
    options.args["autoloot"] = {
      type = "group",
      name = L["Auto-loot Trash"],
      desc = L["Automatically hand off items at or below the chosen quality to a designated player when the loot window opens."],
      order = 72,
      hidden = function() return not (admin()) end,
      args = {
        enable = {
          ...
          desc = L["Automatically hand off items at or below the chosen quality to a designated player when the loot window opens."],
          order = 10,
          get = function() return not not sepgp_autoloot end,
          set = function(v) sepgp_autoloot = not sepgp_autoloot end,
        },
        quality = {
          type = "text",
        ...
        target = {
          type = "text",
          name = L["Auto-loot Target"],
          desc = L["Character to receive auto-looted trash. Leave blank to keep it yourself."],
          order = 30,
          usage = "<CharacterName>",
          get = function() return sepgp_autoloot_target or "" end,
          set = function(v) sepgp_autoloot_target = v end,
        },
      },
    }
    ...
    options.args["progress_tier"] = {
      ...
      validate = { ["T3.5"]="5.Tower of Karazhan", ["T3"]=L["4.Naxxramas"], ["T2.5"]=L["3.Temple of Ahn\'Qiraj"], ["T2"]=L["2.Blackwing Lair"], ["T1"]=L["1.Molten Core"]},
    }
...
function sepgp:AutoLootTrash()
  if not (sepgp_autoloot and self:lootMaster()) then return end

  local maxQuality = tonumber(sepgp_autoloot_quality)
  if not maxQuality then maxQuality = 2 end -- default: Poor/White/Green

  local targetName = sepgp_autoloot_target
  if not targetName or targetName == "" then
    targetName = self._playerName -- default: keep it on yourself
  end

  local raid_idx
  for i = 1, 40 do
    if GetMasterLootCandidate(i) == targetName then
      raid_idx = i
      break
    end
  end
  if not raid_idx then
    self:defaultPrint(string.format(L["Auto-loot: could not find %s among the master loot candidates for this corpse."], targetName))
    return
  end

  -- iterate high-to-low so slot numbers already given away don't shift the ones we haven't checked yet
...
local raidZones = {[L["Molten Core"]]="T1",[L["Onyxia\'s Lair"]]="T1.5",[L["Blackwing Lair"]]="T2",[L["Ahn\'Qiraj"]]="T2.5",[L["Naxxramas"]]="T3",["Tower of Karazhan"]="T3.5"}
local zone_multipliers = {
  ["T3.5"] = {["T3.5"]=1,["T3"]=1,["T2.5"]=0.75,["T2"]=0.5,["T1.5"]=0.25,["T1"]=0.25},
  ["T3"] =   {["T3.5"]=1,["T3"]=1,["T2.5"]=0.75,["T2"]=0.5,["T1.5"]=0.25,["T1"]=0.25},
  ["T2.5"] = {["T3.5"]=1,["T3"]=1,["T2.5"]=1,["T2"]=0.7,["T1.5"]=0.4,["T1"]=0.4},
  ["T2"] =   {["T3.5"]=1,["T3"]=1,["T2.5"]=1,["T2"]=1,["T1.5"]=0.5,["T1"]=0.5},
